...
| OPS-06 | Dependency majors (held Dependabot PRs), each gated by the full suite, ordered least runtime risk first: vitest 3→5 (#4) — DONE, 514/514 unchanged, zero config changes; eslint 9→10 (#6) — DONE, lint clean unchanged; zod 3→4 (#3) — DONE, typecheck + 514/514 + evals unchanged; setup-node 4→7 (#8) — DONE, rebased and all CI checks green; setup-node 4→7 CI check (#8), typescript 5→7 (#2) — BLOCKED: typescript-eslint 8.70 does not support TS 7 (the lint sensor cannot run; tracking issue typescript-eslint#10940), revisit when support lands | 4/5 done, TS 7 blocked |

### OPS-06 evidence (2026-09-20, agent) — vitest 5

`npm install --save-dev vitest@^5.0.1`; the full suite ran green
(514/514) UNCHANGED on the first run — no vitest.config.ts or test-file
changes needed; `npm run check` 7/7 and the verification receipt green.
Process note (honest): the first attempt was wiped by my own
`reset --hard` after a pre-commit format rejection (a plan-row note
inserted via a script wasn't prettier-canonical) — redone with the Edit
tool and formatting upfront.
...
### OPS-06 evidence (2026-09-20, agent) — eslint 10
...
### OPS-06 evidence (2026-09-20, agent) — setup-node 7 + the TS 7 blocker

setup-node 4→7 (#8): the PR's first CI run failed check (windows, fast)
— but that run PREDATED the .gitattributes LF fix; after a
`@dependabot rebase` all five checks went green (windows fast included),
and the PR merged. TS 7 (#2): the bump itself typechecks the whole
codebase CLEAN (the native rewrite handles it), but `npm run check`'
s eslint step fails hard — typescript-eslint 8.70 does not support
TS 7 (tracking issue typescript-eslint#10940). The lint sensor is a
hard gate and is never weakened or skipped, so the TS 7 bump is DEFERRED
